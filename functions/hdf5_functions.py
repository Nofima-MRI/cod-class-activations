...
def get_all_groups_hdf5(hdf5_file, filter_on = None):
	"""
	Get all the groups from the hdf5 file
	...
	try:
		...
		if os.path.exists(hdf5_file):
			...
			with h5py.File(hdf5_file, 'r') as hf:
				...
				# check if filter_on contains a value, if so, we want return only groups that contain a certain dataset
				if filter_on is None:

					# return the keys of the HDF5 file.
					...
					# check for each group if filter_on dataset exists
					for group in hf.keys():

						# check if filter_on dataset is part of the group keys
						if filter_on in hf[group].keys():
							
							# group contains the filter_on dataset, append to list so we can return it later
							groups.append(group)
		...
		# call function again
		return get_all_groups_hdf5(hdf5_file)
	
	except Exception as e:
		logging.warning('Error reading groups from HDF5 file: {}'.format(e))
		exit(1)
